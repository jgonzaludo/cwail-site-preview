...
interface TokenLabEmbedProps {
  className?: string;
}

const TokenLabEmbed: React.FC<TokenLabEmbedProps> = ({ className = '' }) => {
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="embed-encoding" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Encoding
            </label>
            <select
              id="embed-encoding"
          ...
          <div>
            <label htmlFor="embed-text" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Text Input
            </label>
            <textarea
              id="embed-text"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600 min-h-[150px]">
              {loading ? (
                ...
                <div className="overflow-x-auto max-h-32">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                      {result.offsets.slice(0, 20).map((token, index) => (
                        <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-600">
                          <td className="px-3 py-2 text-sm text-gray-900 dark:text-gray-100">
                            {index}
                          </td>
                          <td className="px-3 py-2 text-sm text-gray-900 dark:text-gray-100 font-mono">
                            {token.slice}
                          </td>
                          <td className="px-3 py-2 text-sm text-gray-900 dark:text-gray-100">
                            {result.ids[index]}
